/**
 * @fileoverview Shared D1 query pipeline for all workers.
 *
 ...
 * are injected as parameters rather than imported, so each worker can
 * provide its own version source and TTL configuration.
 */
 ...
 *
 * Negative caching:
 *   - If queryFn returns null, EMPTY_ENVELOPE is stored with negativeTtlMs
 ...
 */
export async function cachedQuery({ cacheKey, cache, entityTag, ttlMs, negativeTtlMs, queryFn, getVersion, ctx }) {
    // ── Promise coalescing ───────────────────────────────────────
    let inflight = cache.pending.get(cacheKey);

    if (!inflight) {
        inflight = _resolve(cacheKey, cache, entityTag, ttlMs, negativeTtlMs, queryFn, getVersion, ctx);
 ...
 */
async function _resolve(cacheKey, cache, entityTag, ttlMs, negativeTtlMs, queryFn, getVersion, ctx) {
    ...
    if (buf === null) {
        ...
        cache.add(cacheKey, EMPTY_ENVELOPE, { entityTag }, Date.now());
        const negWrite = putL2(l2Key, EMPTY_ENVELOPE, negativeTtlMs / 1000);
